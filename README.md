...
- **Admin Overview dashboard (§8.2)**: an admin-only `GET /api/admin/overview` composes a read-only user/draft/engagement summary plus attention alerts (failed emails, paused drafts, accounts awaiting activation) from the existing stores — engagement figures (lobby→start / completion conversion, pick volume, timer auto-pick rate, email-delivery health) are re-derived from the append-only draft-event trail because the §15 analytics meter is write-only. Works identically on both storage branches; no migration.
