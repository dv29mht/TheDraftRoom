...
**Document status:** Draft v0.24 (PR-22 complete)  
**Date:** 16 July 2026  
**Product owner:** TBD  
...
With the full MVP feature set delivered (PR-18–PR-21) and the non-functional release
requirements now met by PR-22 — PWA lifecycle with the version handshake, explicit
offline behavior, install guidance, device ergonomics, observability with correlation
IDs/metrics/error-monitoring seams, and measured §14 performance with WCAG 2.2 AA
evidence — one PR remains. The next implementation session is **PR-23 — end-to-end MVP
verification and private-beta release** (PRD §17.9, §16): complete 1v1 and 2v2
multi-client E2E suites, concurrency/race tests, reconnect and Brevo-outage scenarios,
deployment/runbook documentation, seed/demo data, backup/recovery notes, the retention
policy (§12.3), analytics events (§15), and the private-beta checklist. Done when every
item in §16 has linked automated or recorded manual evidence, repeated real-iPhone and
desktop sessions complete successfully, no release-blocking defect remains, and the
private-beta build is reproducible from a clean environment.
