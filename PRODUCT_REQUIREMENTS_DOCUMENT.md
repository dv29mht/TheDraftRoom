...
**Document status:** Draft v0.18 (PR-11 complete)  
**Date:** 15 July 2026  
**Product owner:** TBD  
...
With the usable 1v1/2v2 lobby now in place (PR-11) — host-created lobbies with a
chosen roster template, an invite list, join/presence, host-only remove and lock,
server-side capacity enforcement (1v1 2–10, 2v2 4–16 even), and the authoritative
snapshot surfaced through the new `DraftsController` and lobby UI — the next
implementation session is **PR-12 — 2v2 seeding, team formation, and ready check**
(PRD §17.6). It makes both formats start-ready under the confirmed rules: host-only
Seed 1/Seed 2 assignment, solo-team projection for 1v1, paired-team formation for
2v2 (exactly one Seed 1 + one Seed 2 per team), ready/unready actions, attendance
confirmation, validation summaries, and the configuration-freeze rules. It builds
directly on PR-11's `Draft`/`DraftParticipant` lobby (now in the `TeamFormation`
state after lock) and the `DraftTeam`/`DraftTeamMember`/`DraftSeed` entities already
present on the aggregate. The existing canonical moodboard and persisted design
