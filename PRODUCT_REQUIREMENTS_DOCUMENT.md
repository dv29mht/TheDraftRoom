...
**Document status:** Draft v0.19 (PR-12 & PR-13 complete)  
**Date:** 15 July 2026  
**Product owner:** TBD  
...
**Deployment note (16 July 2026):** The live deployment runs on **Google Cloud Run** (service `the-draft-room`, region `us-east4`, project `project-71e2bff0-ecd4-4779-a37` / number `909367690008`) backed by **Neon** PostgreSQL — **not** Render, despite the committed `render.yaml`/`DEPLOYMENT.md` (a legacy/alternative path, not the live target).

**Release process — one step promotes to production:** **push / merge to `main` on GitHub**, which runs CI and then **automatically deploys the built image to Google Cloud Run**, so the build goes **live in production** with no separate manual deploy. In other words, git and Google Cloud are kept in lock-step by the pipeline: landing code on `main` *is* the prod release. The mechanism is a GitHub Actions workflow (`.github/workflows/deploy-cloud-run.yml`) that builds the repo `Dockerfile` and runs `gcloud run deploy --source .`, authenticated with keyless **Workload Identity Federation** (no long-lived keys) and gated on the CI workflow passing; the service stays single-instance (`--max-instances 1`) because live draft state is in-memory. EF Core migrations run on boot (`Database__MigrateOnStartup=true`), so schema changes ship with the same push — keep every migration forward-safe. Continuous deployment went live 16 July 2026 (first auto-deploy: the PR #9 merge). §12.1 Hosting updated. No numbered roadmap PR is affected; the next session remains **PR-14**.
...
With both formats now start-ready (PR-12) and a fair, durable spinner order in
place (PR-13) — host-only seeding and team formation (2v2 one-Seed 1 + one-Seed 2
teams, 1v1 solo projection), self-service readiness, a gated Start control, and a
server-authoritative Fisher–Yates spinner revealed by an animated wheel — the next
implementation session is **PR-14 — Five-star club and protected-player round**
(PRD §17.6/§17.7). It completes the pre-draft selection round in spinner order:
ordered club turns, club eligibility/uniqueness rules, club details, protected-player
eligibility (75+, from the chosen club), slot linkage under the PR-01 decision,
availability updates, and the transition into position drafting (SpinnerRanking →
ClubSelection → PositionDraft). It builds directly on PR-13's committed
`DraftTeam.SpinnerRank` order and the frozen `DraftRosterSlot` snapshot, and on the
curated five-star `Club` eligibility from PR-09. The existing canonical moodboard and
persisted design system remain the approved UI direction.
