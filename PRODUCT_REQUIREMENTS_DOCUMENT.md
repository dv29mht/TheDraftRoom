...
**Document status:** Draft v0.20 (PR-14 & PR-15 complete)  
**Date:** 16 July 2026  
**Product owner:** TBD  
...
With the pre-draft club/held round and the position-draft pick engine now complete
(PR-14/PR-15) — straight-order five-star club + protected-player selection, then a
snake-order position draft (ST → GK, 4 subs) with turn/eligibility/uniqueness/
availability all server-enforced through to **Completed** — the next implementation
session is **PR-16 — Server timer and host controls** (PRD §17.7, §6.4–§6.5). It makes
time and exceptional states authoritative: the 120-second server clock with a 15-second
warning, the approved **auto-pick on expiry** (highest overall → name → id — reusing the
pure `DraftTurnOrder` and the eligibility pool already in place), host pause/resume/cancel
with reason, admin recovery authorization, refresh/restart-safe timer restoration, and
compensating audit events. It builds directly on PR-15's `SubmitPickCommand`, the frozen
`DraftRosterSlot` snapshot, and `IDraftCatalog`; real-time propagation (SignalR) is PR-17
and the rich live-draft room is PR-18. The existing canonical moodboard and persisted
design system remain the approved UI direction.
