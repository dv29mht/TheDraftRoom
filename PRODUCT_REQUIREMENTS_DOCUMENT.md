...
**Document status:** Draft v0.21 (PR-16 & PR-17 complete)  
**Date:** 16 July 2026  
**Product owner:** TBD  
...
With time and exceptional states now authoritative (PR-16 — persisted 120s clock, 15s
warning, deterministic auto-pick on expiry, host pause/resume/cancel with audited reasons,
admin-only compensating recovery) and every client synchronized live (PR-17 — the
authenticated `/hubs/draft` SignalR hub broadcasting a version-stamped snapshot after
every accepted mutation, with reconnect reconciliation and connection-status primitives),
the next implementation session is **PR-18 — Live draft room experience** (PRD §17.7,
§9.6, §8.3, §13). It delivers the complete desktop and one-handed iPhone drafting
workflow on top of the live channel that already exists: the active team/position/timer
header, eligible-player search and filters without leaving the room, player detail cards
(stats, roles with `+`/`++`, PlayStyles), a pick confirmation sheet naming team and slot,
the personal shortlist (decision 11), a compact team rail with focused squad / recent
picks / full history views, the sticky mobile action area replacing bottom navigation,
richer connection/reconnect states, and pick/turn motion. Everything server-side it needs
— the board, the pushed `DraftUpdated` snapshots, the timer DTO — is in place; PR-18 is
UI/UX depth, keyboard/touch accessibility (44×44 targets, 375 px), and reduced-motion
equivalents. The existing canonical moodboard and persisted design system remain the
