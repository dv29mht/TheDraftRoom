...
- **Hosting:** one process, port, origin, and deployable bundle, packaged as a single multi-stage Docker image; ASP.NET serves the built React PWA and `/api` endpoints. The reference deployment is a single Render web service backed by a managed Neon PostgreSQL instance; the API binds the platform-injected `PORT` and honors `X-Forwarded-Proto` behind the TLS-terminating edge proxy. The live draft state is in-memory, so the service runs as a single instance and must not scale horizontally.
