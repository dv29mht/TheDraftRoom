...
**Document status:** Draft v0.22 (PR-18, PR-19 & PR-20 complete)  
**Date:** 16 July 2026  
**Product owner:** TBD  
...
With the full player-facing product now delivered — the complete draft room experience
(PR-18), the immutable results and squad archive (PR-19), and persistent per-user
notifications plus outbox-backed draft emails with §9.9 preferences (PR-20) — the next
implementation session is **PR-21 — Admin communications, draft operations, and audit
views** (PRD §17.8, §9.8's admin half, §9.10, §8.2). It completes MVP administration and
recovery: announcement audience/preview/confirmation with Brevo campaign metadata over
the existing outbox (respecting the PR-20 `optional_email_opt_out` preference — the
announcement is exactly the "optional" class that flag exists for), admin draft
inspect/pause/resume/cancel operations building on the PR-16 commands and admin-only
recovery, immutable audit-log queries over the append-only `DraftEvent` and
security-audit stores, reason capture, delivery visibility via the outbox status reader,
and the role-aware Admin-versus-Player context. Done when bulk sends are confirmed and
throttled, every admin/host action is attributable, audit records cannot be edited or
deleted through normal APIs, and recovery uses compensating events. After PR-21 only the
release-hardening PRs (PR-22 PWA/accessibility/observability, PR-23 end-to-end MVP
verification) remain.
