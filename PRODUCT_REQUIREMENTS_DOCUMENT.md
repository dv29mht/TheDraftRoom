...
**Document status:** Draft v0.23 (PR-21 complete)  
**Date:** 16 July 2026  
**Product owner:** TBD  
...
With the full MVP feature set now delivered — the complete player-facing product
(PR-18–PR-20) and the admin communications, draft operations, and audit views that
complete administration and recovery (PR-21) — only the release-hardening PRs remain.
The next implementation session is **PR-22 — PWA lifecycle, accessibility, performance,
and observability** (PRD §17.9, §12.2, §14, §15): offline/reconnecting
behavior that blocks offline mutations, install and iOS guidance, the service-worker
update prompt and version handshake (so a cached shell never runs against an
incompatible API — §18 risk), safe-area/orientation/keyboard handling, structured logs
with correlation IDs, metrics and error-monitoring hooks, and the
performance/accessibility fixes. Done when core journeys pass automated accessibility
checks plus manual keyboard, reduced-motion, light/dark contrast, 44×44 px touch-target,
375 px, landscape, and safe-area reviews; authenticated API data is never cached; and
measured performance is recorded against §14. After PR-22, PR-23 (end-to-end MVP
verification and the private-beta release) closes out the MVP.
